...
void SceneManager::LoadSceneTextures()
{
	...
	bReturn = CreateGLTexture("../../Utilities/textures/background.jpg", "white");
	...
	bReturn = CreateGLTexture("../../Utilities/textures/can2.jpg", "can2");
	bReturn = CreateGLTexture("../../Utilities/textures/deskwood.jpg", "deskwood");
	bReturn = CreateGLTexture("../../Utilities/textures/key2.png", "key2");
	//pre workout textures
...
//	m_pShaderManager->setVec3Value("lightSources[1].specularColor", 0.2f, 0.2f, 0.2f);
	...
	m_pShaderManager->setFloatValue("lightSources[2].focalStrength", 12.0f);
	m_pShaderManager->setFloatValue("lightSources[2].specularIntensity", 0.2f);
	// New light source for the monitor glow
	m_pShaderManager->setVec3Value("lightSources[3].position", 0.0f, 6.40f, -5.90f); // Position near the monitor screen
	m_pShaderManager->setVec3Value("lightSources[3].ambientColor", 0.05f, 0.05f, 0.2f); // Low blue ambient glow
	m_pShaderManager->setVec3Value("lightSources[3].diffuseColor", 0.1f, 0.1f, 0.8f);  // Brighter blue diffuse light
	m_pShaderManager->setVec3Value("lightSources[3].specularColor", 0.2f, 0.2f, 1.0f); // Slight specular highlight
	m_pShaderManager->setFloatValue("lightSources[3].focalStrength", 16.0f);           // Medium focal strength
...
{	
	...
	SetShaderMaterial("metal"); // reflective plane 
	...
	// Body of pre workout
	scaleXYZ = glm::vec3(1.15f, 1.0f, 1.15f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(5.0f,1.03f, -3.0f);
	...
	// draw the mesh with transformation 
	...
	// label of pre workout
	scaleXYZ = glm::vec3(1.17f, 0.55f, 1.17f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(5.0f, 1.38f, -3.0f);
	...
	// Lid of the pre workout
	scaleXYZ = glm::vec3(1.10f, 0.25f, 1.10f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(5.0f, 2.03, -3.0f);
	...
	/****************************************************************/
		// desk
	scaleXYZ = glm::vec3(15.0f,1.0f, 7.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 0.50f, -3.0f);
	...
	// monitor screen
	scaleXYZ = glm::vec3(11.0f, 5.50f, 0.2f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 6.40f, -5.70f);
	...
	/****************************************************************/
	// monitor
	scaleXYZ = glm::vec3(12.0f, 7.0f, 0.4f);
	...
	// monitor stand body
	scaleXYZ = glm::vec3(3.0f, 4.0f, 0.20);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 3.10f, -6.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 1.10f, -5.40);
	...
	//keyboard base
	scaleXYZ = glm::vec3(4.0f, 0.1f, 1.25f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 1.09f, -2.0f);
	...
	scaleXYZ = glm::vec3(3.5f, 0.09f, 1.10f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(0.0f, 1.10f, -2.0f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-5.0f, 1.02f, -3.0f);
	...
	SetShaderColor(0.9f, 0.7f, 0.2f, 1.0f);
	SetShaderTexture("metal");
	// draw the mesh with transformation values
	SetShaderTexture("can2");
	m_basicMeshes->DrawCylinderMesh();

	/****************************************************************/
	
	//another can top 
	scaleXYZ = glm::vec3(0.30f, 0.1f, 0.40f);
	...
	// set the XYZ position for the mesh
	positionXYZ = glm::vec3(-5.0f, 3.49f, -3.2f);
	...
	SetShaderColor(0.5f, 0.2f, 0.8f, 1.0f);
	SetShaderTexture("metal");
	// draw the mesh with transformation values
